/*********************************************************************************************************************
 ** File: nesl_la.h
 ** Abstract: RTW Implementation of the Linear Algebra Service for deployed NE
 *solver
 **
...
size_t rtw_linalg_memusage(const McLinearAlgebra* ne_la)
{
    ...
    return
        ...
        ne_la_data->mNumRow * ne_la_data->mNumCol * sizeof(real_T) +
        ne_la_data->mNumRow * sizeof(real_T) +
        ((ne_la_data->mNumRow < ne_la_data->mNumCol) ?
         ne_la_data->mNumRow : ne_la_data->mNumCol) * sizeof(int32_T);
}
                                            ...
                                            const PmSparsityPattern*      pattern)
{
    ...
    if(N <= 20.0 || (N <= 100.0 && nnz/N > 0.1125*N - 1.25) || (N > 100 && nnz/N > 0.277*N - 17.7)) {
        result = full_la->mCreateLinearAlgebra(full_la, linAlg, pattern);
    } else {
        result = sparse_la->mCreateLinearAlgebra(sparse_la, linAlg, pattern);
    }
